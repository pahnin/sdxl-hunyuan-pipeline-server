...
def move_model_to_device_with_memory_preservation(
    ...
    _incremental_move(model, target_device, preserved_memory_gb)
    return


def _incremental_move(model, target_device, preserved_memory_gb):
    """move with batching to avoid hanging up"""
    moved = 0
    batch_size = 50  # Process 50 modules at once
    ...
    for i in range(0, len(modules_list), batch_size):
        ...
        torch.cuda.synchronize()  # Wait for async
