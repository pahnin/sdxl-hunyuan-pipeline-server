...
class OriginalModelGenerator(BaseModelGenerator):
    ...
    def load_model(self):
        ...
        print(f"Loading {self.model_name} Transformer... {self.model_path}")

        path_to_load = self.model_path  # Initialize with the default path

        if self.offline:
            path_to_load = (
                self._get_offline_load_path()
            )  # Calls the method in BaseModelGenerator
        ...
        if not self.high_vram:
            print(
                f"[GENERATOR_PRE_HUNYUAN] RAM={process.memory_info().rss/1e9:.2f}GB CPU={process.cpu_percent():.1f}%"
            )
            DynamicSwapInstaller.install_model(
                self.transformer, device=self.gpu
            )  # line 52
            print(
                f"[GENERATOR_POST_HUNYUAN] RAM={process.memory_info().rss/1e9:.2f}GB CPU={process.cpu_percent():.1f}%"
            )
        else:
    ...
    def get_current_pixels(
        ...
        return vae_decode_with_tiling(
            real_history_latents[:, :, :section_latent_frames],
            vae,
            low_vram_tiling_enabled=low_vram_tiling_enabled,
        ).cpu()
